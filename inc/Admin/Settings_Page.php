...
class Settings_Page {
    ...
    public function register_menu() {
        add_submenu_page(
            'joinotify-workflows',
            esc_html__( 'Configurações OTP Login', 'joinotify-otp-login' ),
            esc_html__( 'Configurações OTP Login', 'joinotify-otp-login' ),
            'manage_options',
            self::PAGE_SLUG,
    ...
    public function render_page() {
        if ( ! current_user_can( 'manage_options' ) ) {
            wp_die( esc_html__( 'Você não tem permissão para acessar esta página.', 'joinotify-otp-login' ) );
        }
    ...
    public function get_border_radius() {
        return $this->sanitize_border_radius( get_option( 'joinotify_otp_login_border_radius', 28 ) );
    }
